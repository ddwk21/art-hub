...
function renderHarvard() {
  ...
  if (harvardWorking.length <= 1||art.classificationid == 17) {
    ...
    if (!!art.title)$("#info"+elementCount).append(`<p>Title: ${art.title}</p>`);

    if (!!art.people.name)$("#info"+elementCount).append(`<p>Name: ${art.people[0].name}</p>`);

    if (!!art.people.role)$("#info"+elementCount).append(`<p>Role: ${art.people[0].role}</p>`);

    if (!!art.dated)$("#info"+elementCount).append(`<p>Dated: ${art.dated}</p>`);

    if (!!art.dated)$("#info"+elementCount).append(`<p>Culture: ${art.culture}</p>`)
...
function addContent(api)
{
    ...
    $('#body').append('<div class="art-container" id = "container'+elementCount+'"></div>')

    //generate new img element
    $('#art-container').append('<img class="mx-auto art-image my-11" id = "image"' +elementCount+ 'alt="Art Image"/>')

    $('#info').append('<div class="overlay"><div class="art-info max-w-1/2" id="info'+elementCount+'"></div></div>')
